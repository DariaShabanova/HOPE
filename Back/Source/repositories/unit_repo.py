...
from Source.utils import unite, unite_for_return
...
class UnitRepository:  # класс для работы с бд (отправки запросов в бд)
    ...
    async def get_all(self, *args, **kwargs) -> Sequence:
        ...
        query = select(
            ...
            d.amount.label("delivery_amount"), d.price_per_instance.label("delivery_price_per_instance"),
            d.quantity.label("delivery_quantity"), d.total.label("delivery_total"),
            r.amount.label("return_amount"), r.price_per_instance.label("return_price_per_instance"),
            r.quantity.label("return_quantity"), r.total.label("return_total")
        ).select_from(i)
        query = query.join(d, i.id == d.order_id)
        query = query.join(r, i.id == r.order_id, isouter=True)
        result = await self.session.execute(query)  # tекстовое выполнение запроса
        return tuple(result.all())  # получим кортеж последовательность строчек в бд
    ...
    async def insert_many_item(self, datas: list[OzonRow]) -> Sequence:
        ...
        return result.scalars().all()  # tекстовое выполнение запроса
    ...
    async def insert_many_return(self, datas: list[OzonRow], order_ids: list[int]):
        return_commissions = [data.return_commission.model_dump() if data.return_commission else None for data in datas]
        datus = unite_for_return(return_commissions, order_ids)
